...
class SimpleTurbine(BaseComponent):
    """Analytic turbine: fixed expansion ratio + isentropic efficiency.

    PR is the expansion ratio P_in/P_out (>1). No performance map — the ideal
    (isentropic) outlet temperature is the textbook ideal-gas relation
    T2s = T1 * (1/PR)**((gamma-1)/gamma); the actual enthalpy drop is that
    isentropic drop scaled down by eta_s (inefficiency means less work is
    extracted than the ideal expansion would give). gamma: give it directly,
    or leave it None (the default) to derive it from the network's own
    fluid model instead, via BaseFluid.gamma(P_in, T_in) evaluated fresh at
    each residual call — every fluid model here implements cp()/cv() (see
    ...
    fluid loses to something else. None (the default) means fully
    adiabatic, unchanged from before this existed — see Turbine's
    ...
    def __init__(
        ...
        if PR <= 0:
            raise ValueError(f"SimpleTurbine {name!r}: PR must be > 0, got {PR}")
        if not (0.0 < eta_s <= 1.0):
            raise ValueError(f"SimpleTurbine {name!r}: eta_s must be in (0, 1], got {eta_s}")
        if gamma is not None and gamma <= 1.0:
            raise ValueError(f"SimpleTurbine {name!r}: gamma must be > 1, got {gamma}")
        self.name = name
        self.PR = PR
        self.eta_s = eta_s
        self.gamma = gamma
    ...
    def residuals(self, state: "NetworkState") -> list[float]:
        ...
        T_out_isentropic = T_in * (1.0 / self.PR) ** ((gamma - 1.0) / gamma)
